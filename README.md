...
Index source files for semantic code search using **E5-small** embeddings, with a
pluggable vector backend: **Qdrant** (Cloud server-side inference, default) or
**DuckDB** (local VSS/HNSW) — and, for DuckDB, a pluggable embedder: **ort** (raw
ONNX Runtime, default) or **ollama** (remote HTTP).
...
- TypeScript today; Go (or any C-family language) by changing two flags.

## Backends

| Backend | Embeddings | Storage | Build | Network on first run |
| ------- | ---------- | ------- | ----- | -------------------- |
| **qdrant** (default) | Qdrant Cloud **server-side inference** (`Document` API — no local model) | Qdrant Cloud collection | `--features qdrant` (default) | none locally; needs the cluster |
| **duckdb** | local, via an **embedder** (see below) | DuckDB file + **VSS/HNSW** cosine index | `--features ort` / `--features ollama` (or `all`) | embedder-dependent + the DuckDB VSS extension |
...
| -------- | --- | ----- | -------------------- |
| **ort** (default) | raw **ONNX Runtime** (`ort` 2.x) + tokenizer; downloads `onnx/model.onnx` + `tokenizer.json` from `duckdb.model_repo` (default `Xenova/multilingual-e5-small`, 384d) | `--features ort` | downloads the ONNX model + tokenizer from HuggingFace |
| **ollama** | remote **Ollama** HTTP server: `POST {ollama.url}/api/embed` | `--features ollama` | none to download — but needs a running Ollama with the model pulled |
...
- One chunk per top-level **function** / arrow-or-function **const** / **class** /
  **interface** / **type-alias**, with the identifier stored as the chunk's `symbol`.
- An **oversized class** (text > `max_chunk_chars`) is split into one chunk per method
  (symbol `Class.method`); an **oversized function** is line-split, keeping its symbol.
- Lines not under any captured node (imports, top-level statements) become **remainder**
  chunks (no symbol) — **nothing is dropped**.
...
```bash
# Default build = Qdrant only.
cargo build --release

# DuckDB backend + local ONNX embedder (bundled DuckDB + ort/ONNX runtime).
cargo build --release --features ort

# DuckDB backend + Ollama HTTP embedder (no local ONNX deps).
cargo build --release --features ollama

# AST (tree-sitter) chunker — symbol-aware chunks for TS/TSX.
cargo build --release --features ast

# Everything (qdrant + ort + ollama + ast).
cargo build --release --features all
```

The binary lands at `./target/release/semanticastindexer`. The `ort`
feature pulls in bundled DuckDB and ONNX Runtime (`ort`), so it builds slower and
produces a larger binary — only enable it when you want the local backend. Feature
flags: `qdrant` (default), `duckdb` (storage only), `ort` (= duckdb + ONNX embedder),
`ollama` (= duckdb + HTTP embedder), `ast` (= tree-sitter chunker, backend-free),
`all` (= qdrant + ort + ollama + ast).
...
`--config /path/to/indexer.yaml`. With a `--features ort` build you can index any project
...
# 2. Index the TypeScript tree (creates the collection/table if missing).
"$BIN" --root src --ext ts,tsx --language ts --collection source_code

# 3. Index Go later, into the same collection.
"$BIN" --root path/to/go --ext go --language go --collection source_code
...
| `--ext <list>` | `ts,tsx` | Comma-separated extensions (no dots). |
| `--language <s>` | `ts` | Label stored in each point's payload. |
...
| `--model <s>` | config / `intfloat/multilingual-e5-small` | Inference model (overrides config). |
...
client needed. Both subcommands need a vector backend **and** an embedder feature (build
with `--features ort`, `--features "ollama,ast"`, or `--features all`); with a backend whose
feature was not compiled in you get the usual clear "rebuild with --features …" error. The
index is opened **read-only** (so a search can run while an index is open elsewhere). The
top-level `--backend` / `--embedder` / `--collection` / `--config` flags still apply (before
or after the subcommand) and pick up the YAML defaults.
...
vector_dim: 384      # MUST match the embedder model (e5-small=384, nomic=768, mxbai=1024)

duckdb:              # backend: duckdb settings
  path: .index/code.duckdb
  # model_repo: Xenova/multilingual-e5-small   # ort embedder HF repo
...
Build with `--features all` (or at least `mcp`, which pulls in `duckdb` + `ollama`) and
