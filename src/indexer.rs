...
//! Every path here is PURE (`collect_chunks`, `build_chunks`, `dry_run`) — it never touches
//! a vector backend. The per-file re-index step (delete + re-chunk + upsert) lives in
//! [`crate::service::IndexingService`], which calls [`collect_chunks`]/[`build_chunks`] here.
...
/// to produce chunks from this file?". Used by `collect_chunks`, `dry_run`, and
/// `IndexingService::reindex_file` (and therefore both CLI sync and MCP refresh).
///
