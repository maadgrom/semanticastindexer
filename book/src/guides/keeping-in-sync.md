...
- **File exists and is indexable** (wanted extension, passes the `indexer.yaml` globs, not an
...
merged. `sync` respects the same `--ext` and `indexer.yaml` filters as a full index.
