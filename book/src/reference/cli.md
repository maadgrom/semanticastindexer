...
| `-v, --verbose` | flag | `false` | Increase log verbosity (repeatable: `-v` → `debug`, `-vv` → `trace`). Default is `info` level. |
| `--log-format <s>` | string | `pretty` | Log format: `pretty` (human-readable) or `json` (structured). Ignored by `--silent`. |
| `--silent` | flag | `false` | Suppress logs (sets level to `error`), timing, progress, dirty warnings, and non-essential notes. Ideal for hooks/CI. See [logging](#logging) and [timing & output](#timing-and-output). |

> Note: `--backend`, `--embedder`, `--config`, `--collection`, `-v/--verbose`, `--log-format`, and `--silent` are global. The index-only flags below (`--root`, `--ext`, `--chunker`, `--model`, `--query`, `--query-only`, `--recreate`, `--dry-run`, `--limit`) are **not** global — they belong to the default (no-subcommand) index action.
...
1. `RUST_LOG` environment variable (e.g. `RUST_LOG=semanticastindexer=debug`)
2. CLI flags: `-v` / `-vv` → `debug` / `trace`; `--silent` → `error`
3. Default: `info`
...
semanticastindexer -vv --query "my query"

# Suppress logs (error level only).
semanticastindexer --silent
```

**For MCP clients:** diagnostics go to stderr and are captured in the client's log file. The JSON-RPC stream (stdout) remains clean, enabling write-mode (`--allow-write`) even with strict JSON-RPC clients (e.g. Grok).
