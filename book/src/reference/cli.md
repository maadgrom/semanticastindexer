...
| `--silent` | flag | `false` | Suppress logs (sets level to `error`), per-op timing, progress, dirty warnings, and non-essential notes. Ideal for hooks/CI. See [logging](#logging) and [timing & output](#timing-and-output). |

> Note: `--backend`, `--embedder`, `--config`, `--collection`, `-v/--verbose`, `--log-format`, `--timing`, and `--silent` are global. The index-only flags below (`--root`, `--ext`, `--chunker`, `--model`, `--query`, `--query-only`, `--recreate`, `--dry-run`, `--limit`) are **not** global — they belong to the default (no-subcommand) index action.
...
Three knobs — **level**, **format**, and **timing** — each resolve in the same order: the `RUST_LOG` env var / CLI flag first, then the project config's [`logging:`](configuration.md#logging) block, then a built-in default.
...
**Timing** controls only the per-operation span-close durations (index/embed/query/sync). The single end-of-command summary line (`done … in Ns`) is a plain `info` event and prints regardless — see [timing & output](#timing-and-output).
...
Project-wide defaults can live in `sai-cfg.yml` instead of flags — see the [`logging:` config block](configuration.md#logging). The flags and `RUST_LOG` always override it.
...
- [Configuration reference](../reference/configuration.md) — every `sai-cfg.yml` key.
